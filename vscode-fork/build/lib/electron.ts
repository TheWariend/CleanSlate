...
function darwinBundleDocumentType(extensions: string[], icon: string, nameOrSuffix?: string | DarwinDocumentSuffix, utis?: string[]): DarwinDocumentType {
	...
	const iconPath = 'resources/darwin/' + icon.toLowerCase() + '.icns';
	const finalIconPath = fs.existsSync(path.join(root, iconPath)) ? iconPath : 'resources/darwin/code.icns';
...
function darwinBundleDocumentTypes(types: { [name: string]: string | string[] }, icon: string): DarwinDocumentType[] {
	return Object.keys(types).map((name: string): DarwinDocumentType => {
		...
		const iconPath = 'resources/darwin/' + icon + '.icns';
		const finalIconPath = fs.existsSync(path.join(root, iconPath)) ? iconPath : 'resources/darwin/code.icns';
		return {
			name,
			role: 'Editor',
...
export const config = {
	...
	darwinIcon: fs.existsSync(path.join(root, 'resources/darwin/CleanSlate.icns')) ? 'resources/darwin/CleanSlate.icns' : 'resources/darwin/code.icns',
