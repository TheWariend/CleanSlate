...
const resolvedCommit = commit ?? cp.execSync('git rev-parse HEAD', { cwd: repoPath, encoding: 'utf8' }).trim();
...
function toWinePath(value: string): string {
	if (process.platform !== 'darwin' || !path.isAbsolute(value)) {
		return value;
	}

	return cp.execFileSync('winepath', ['-w', value], { encoding: 'utf8' }).trim();
}
...
function packageInnoSetup(iss: string, options: { definitions?: Record<string, unknown> }, cb: (err?: Error | null) => void) {
	...
	const defs = keys.map(key => `/d${key}=${toWinePath(String(definitions[key]))}`);
	const args = [
		toWinePath(iss),
		...defs,
		`/sesrp=node ${toWinePath(signWin32Path)} $f`
	];

	const command = process.platform === 'darwin' ? 'wine' : innoSetupPath;
	const commandArgs = process.platform === 'darwin' ? [toWinePath(innoSetupPath), ...args] : args;
...
function buildWin32Setup(arch: string, target: string): task.CallbackTask {
	...
	return (cb) => {
		...
		let versionedResourcesFolder = useVersionedUpdate ? resolvedCommit.substring(0, 10) : '';
		const issPath = path.join(import.meta.dirname, 'win32', 'code.iss');
		let originalProductJsonPath = path.join(sourcePath, versionedResourcesFolder, 'resources/app/product.json');
		if (versionedResourcesFolder && !fs.existsSync(originalProductJsonPath)) {
			const unversionedProductJsonPath = path.join(sourcePath, 'resources/app/product.json');
			if (fs.existsSync(unversionedProductJsonPath)) {
				console.warn(`Versioned Windows resources folder not found at ${path.join(sourcePath, versionedResourcesFolder)}; building setup from unversioned app output.`);
				versionedResourcesFolder = '';
				originalProductJsonPath = unversionedProductJsonPath;
			}
		}
		...
		const shouldBuildAppxPackage = (quality === 'stable' || quality === 'insider') && Boolean((product as { win32ContextMenu?: unknown }).win32ContextMenu);

		if (shouldBuildAppxPackage) {
