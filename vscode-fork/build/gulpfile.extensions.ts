...
const tasks = compilations.map(function (tsconfigFile) {
	...
	const compileTask = task.define(`compile-extension:${name}`, task.series(cleanTask, async () => {
		...
		const tsgo = spawnTsgo(absolutePath, () => rewriteTsgoSourceMappingUrlsIfNeeded(false, out, baseUrl), { label: name });
	...
	const watchTask = task.define(`watch-extension:${name}`, task.series(cleanTask, () => {
		...
		const tsgoStream = watchInput.pipe(util.debounce(() => createTsgoStream(absolutePath, () => rewriteTsgoSourceMappingUrlsIfNeeded(false, out, baseUrl), { label: name }), 200));
...
// Diagnostic helper: run extension compiles serially so the failing target is explicit in logs.
export const compileExtensionsSerialTask = task.define('compile-extensions-serial', task.series(...tasks.map(t => t.compileTask)));
gulp.task(compileExtensionsSerialTask);
