...
const vscodeResourceIncludes = [
	...
	'out-build/vs/workbench/services/cleanSlate/node/core/cleanSlateCliAgentRuntime*.js',

	// Touchbar
	'out-build/vs/workbench/browser/parts/editor/media/*.png',
	'out-build/vs/workbench/contrib/debug/browser/media/*.png',
	'out-build/vs/workbench/contrib/cleanSlate/browser/media/*.{png,svg}',
	...
	const task = () => {
		...
		if (platform === 'win32') {
			all = es.merge(all, gulp.src([
				...
				'resources/win32/CleanSlate.png',
		...
		if (platform === 'win32') {
			...
			if (useVersionedUpdate) {
				result = es.merge(result, gulp.src('resources/win32/versioned/bin/code.cmd', { base: 'resources/win32/versioned', allowEmpty: true })
					.pipe(replace('@@NAME@@', product.nameShort))
					.pipe(replace('@@VERSIONFOLDER@@', versionedResourcesFolder))
					.pipe(rename(function (f) { f.basename = product.applicationName; })));

				result = es.merge(result, gulp.src('resources/win32/versioned/bin/code.sh', { base: 'resources/win32/versioned', allowEmpty: true })
					.pipe(replace('@@NAME@@', product.nameShort))
				...
				result = es.merge(result, gulp.src('resources/win32/bin/code.cmd', { base: 'resources/win32', allowEmpty: true })
					.pipe(replace('@@NAME@@', product.nameShort))
					.pipe(rename(function (f) { f.basename = product.applicationName; })));

				result = es.merge(result, gulp.src('resources/win32/bin/code.sh', { base: 'resources/win32', allowEmpty: true })
					.pipe(replace('@@NAME@@', product.nameShort))
					.pipe(replace('@@PRODNAME@@', product.nameLong))
					.pipe(replace('@@VERSION@@', version))
					.pipe(replace('@@COMMIT@@', String(commit)))
			...
			result = es.merge(result, gulp.src('resources/win32/VisualElementsManifest.xml', { base: 'resources/win32', allowEmpty: true })
