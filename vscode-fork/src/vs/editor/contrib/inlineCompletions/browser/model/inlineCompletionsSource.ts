...
import { DataChannelForwardingTelemetryService, forwardToChannelIf, isAILikeExtension } from '../../../../../platform/dataChannel/browser/forwardingTelemetryService.js';
...
export class InlineCompletionsSource extends Disposable {
	...
	private _sendInlineCompletionsRequestTelemetry(
		...
		if (!requestResponseInfo.providers.some(p => isAILikeExtension(p.providerId?.extensionId))) {
			return;
		}
		...
		const emptyEndOfLifeEvent: InlineCompletionEndOfLifeEvent = {
			...
			...forwardToChannelIf(requestResponseInfo.providers.some(p => isAILikeExtension(p.providerId?.extensionId))),
