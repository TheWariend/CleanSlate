...
export class ExtensionManagementService extends AbstractExtensionManagementService implements INativeServerExtensionManagementService {
	...
	private async downloadExtension(extension: IGalleryExtension, operation: InstallOperation, verifySignature: boolean, clientTargetPlatform?: TargetPlatform): Promise<{ readonly location: URI; readonly verificationStatus: ExtensionSignatureVerificationCode | undefined }> {
		...
		if (verifySignature && this.isOpenVSXGallery()) {
			this.logService.info(`Skipping extension signature verification for Open VSX extension: ${extension.identifier.id}`);
	...
	private isOpenVSXGallery(): boolean {
