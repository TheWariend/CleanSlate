...
export class ConsoleMainLogger extends AbstractLogger implements ILogger {
	...
	private safeConsoleWrite(write: (...data: unknown[]) => void, ...data: unknown[]): void {
		try {
			write.apply(console, data);
		} catch (error) {
	...
	trace(message: string, ...args: unknown[]): void {
		if (this.canLog(LogLevel.Trace)) {
			if (this.useColors) {
				this.safeConsoleWrite(console.log, `\x1b[90m[main ${now()}]\x1b[0m`, message, ...args);
			} else {
				this.safeConsoleWrite(console.log, `[main ${now()}]`, message, ...args);
			}
		}
	}
	...
	warn(message: string | Error, ...args: unknown[]): void {
		if (this.canLog(LogLevel.Warning)) {
			if (this.useColors) {
				this.safeConsoleWrite(console.warn, `\x1b[93m[main ${now()}]\x1b[0m`, message, ...args);
			} else {
				this.safeConsoleWrite(console.warn, `[main ${now()}]`, message, ...args);
			}
		}
	}
	...
	error(message: string, ...args: unknown[]): void {
		if (this.canLog(LogLevel.Error)) {
			if (this.useColors) {
				this.safeConsoleWrite(console.error, `\x1b[91m[main ${now()}]\x1b[0m`, message, ...args);
			} else {
				this.safeConsoleWrite(console.error, `[main ${now()}]`, message, ...args);
			}
		}
	}
