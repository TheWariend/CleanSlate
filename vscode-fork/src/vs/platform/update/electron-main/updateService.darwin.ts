...
export class DarwinUpdateService extends AbstractUpdateService implements IRelaunchHandler {
	...
	constructor(
		...
		// supportsUpdateOverwrite is intentionally false on macOS. The Squirrel.Mac feed
		// (createDarwinUpdate in the CleanSlate update server) only carries { url, name, notes,
		// pub_date } and cannot round-trip the build commit. As a result the pending update's
		// `version` is Squirrel's release-notes string, not the git SHA, so checkForOverwriteUpdates
		// -> isLatestVersion never matches manifest.commit, never gets a 204, and always concludes a
		// "newer" update exists. That made both the Ready-state 5-minute timer and the first
		// quitAndInstall re-download the same update in a loop. win32 keeps overwrite (its feed
		// carries version: manifest.commit).
