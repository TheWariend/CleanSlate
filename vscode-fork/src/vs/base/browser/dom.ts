...
/**
 * Returns whether the element is in the bottom left quarter of the container.
 *
 * @param element the element to check for being in the bottom left quarter
 * @param container the container to check against
 * @returns true if the element is in the bottom left quarter of the container
 */
export function isElementInBottomLeftQuarter(element: HTMLElement, container: HTMLElement): boolean {
	const position = getDomNodePagePosition(element);
	const clientArea = getClientArea(container);
