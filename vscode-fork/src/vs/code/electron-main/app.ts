...
import { ICleanSlateVectorStore, IMCPClientService, ICleanSlateMainService, ICleanSlateIndexService } from '../../workbench/services/cleanSlate/common/core/cleanSlateAI.js';
import { NodeCleanSlateVectorStore } from '../../workbench/services/cleanSlate/node/indexing/cleanSlateVectorStore.js';
import { CleanSlateVectorStoreChannel } from '../../workbench/services/cleanSlate/node/indexing/cleanSlateVectorStoreChannel.js';
import { CleanSlateIndexChannel } from '../../workbench/services/cleanSlate/node/indexing/cleanSlateIndexChannel.js';
import { NodeCleanSlateIndexService } from '../../workbench/services/cleanSlate/node/indexing/cleanSlateNodeIndexService.js';
import { CleanSlateMCPChannel } from '../../workbench/services/cleanSlate/node/mcp/cleanSlateMCPChannel.js';
import { CleanSlateMCPClientService } from '../../workbench/services/cleanSlate/node/mcp/cleanSlateMCPClient.js';
import { NodeCleanSlateMainService } from '../../workbench/services/cleanSlate/node/core/cleanSlateMainService.js';
import { CleanSlateMainChannel } from '../../workbench/services/cleanSlate/node/core/cleanSlateMainChannel.js';
import '../../workbench/services/cleanSlate/node/core/cleanSlateNode.contribution.js';
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// CleanSlate Services
		services.set(ICleanSlateVectorStore, new SyncDescriptor(NodeCleanSlateVectorStore));
		services.set(IMCPClientService, new SyncDescriptor(CleanSlateMCPClientService));
		services.set(ICleanSlateIndexService, new SyncDescriptor(NodeCleanSlateIndexService));
		services.set(ICleanSlateMainService, new SyncDescriptor(NodeCleanSlateMainService));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		try {
			...
			this.logService.error('CleanSlate: Failed to register cleanSlateMain IPC channel', e);
		}

		try {
			const mcpService = accessor.get(IMCPClientService);
			mainProcessElectronServer.registerChannel('cleanSlateMCP', new CleanSlateMCPChannel(mcpService));
			this.logService.info('CleanSlate: cleanSlateMCP IPC channel registered successfully.');
		} catch (e) {
			this.logService.error('CleanSlate: Failed to register cleanSlateMCP IPC channel', e);
		}

		try {
			const vectorStore = accessor.get(ICleanSlateVectorStore);
			mainProcessElectronServer.registerChannel('cleanSlateVectorStore', new CleanSlateVectorStoreChannel(vectorStore));
			this.logService.info('CleanSlate: cleanSlateVectorStore IPC channel registered successfully.');
		} catch (e) {
			this.logService.error('CleanSlate: Failed to register cleanSlateVectorStore IPC channel', e);
		}

		try {
			const indexService = accessor.get(ICleanSlateIndexService);
			mainProcessElectronServer.registerChannel('cleanSlateIndex', new CleanSlateIndexChannel(indexService));
			this.logService.info('CleanSlate: cleanSlateIndex IPC channel registered successfully.');
		} catch (e) {
			this.logService.warn('CleanSlate: cleanSlateIndex IPC channel unavailable; semantic indexing will degrade gracefully until the index backend is available.', e);
		}
	}
