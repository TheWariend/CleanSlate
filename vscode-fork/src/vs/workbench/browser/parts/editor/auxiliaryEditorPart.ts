...
export class AuxiliaryEditorPart {
	...
	async create(label: string, options?: IAuxiliaryEditorPartOpenOptions): Promise<ICreateAuxiliaryEditorPartResult> {
		...
		const hideTitlebar = Boolean(options?.hideTitlebar);
		...
		const useCustomTitle = !hideTitlebar && isNative && hasCustomTitlebar(this.configurationService); // custom title in aux windows only enabled in native
		if (useCustomTitle) {
