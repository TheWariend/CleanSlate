...
import { GlobalActivityActionViewItem } from '../globalCompositeBar.js';
...
export class BrowserTitlebarPart extends Part implements ITitlebarPart {
	...
	private registerListeners(targetWindowId: number): void {
		...
		this._register(this.actionViewItemService.onDidChange(menuId => {
			if (menuId === MenuId.TitleBar && hasCustomTitlebar(this.configurationService, this.titleBarStyle) && this.actionToolBar) {
				this.createActionToolBarMenus({ globalActions: true });
			}
		}));
	}
	...
	private actionViewItemProvider(action: IAction, options: IBaseActionViewItemOptions): IActionViewItem | undefined {
		...
		if (!this.isAuxiliary) {
			if (action.id === GLOBAL_ACTIVITY_ID) {
				return this.instantiationService.createInstance(GlobalActivityActionViewItem, () => [], { hoverOptions: { position: () => HoverPosition.BELOW }, colors: () => ({}) }, () => undefined);
			}
		}

		const registeredActionViewItem = this.actionViewItemService.lookUp(MenuId.TitleBar, action.id)?.(action, options, this.instantiationService, getWindow(this.element).vscodeWindowId);
		if (registeredActionViewItem) {
			return registeredActionViewItem;
		}
	...
	private createActionToolBar(): void {
		...
		this.actionToolBar = this.actionToolBarDisposable.add(this.instantiationService.createInstance(WorkbenchToolBar, this.actionToolBarElement, {
			...
			overflowBehavior: { maxItems: 9, exempted: [GLOBAL_ACTIVITY_ID, ...EDITOR_CORE_NAVIGATION_COMMANDS] },
	...
	private get activityActionsEnabled(): boolean {
		...
		return !this.isCompact && !this.isAuxiliary && (activityBarPosition === ActivityBarPosition.TOP || activityBarPosition === ActivityBarPosition.BOTTOM || activityBarPosition === ActivityBarPosition.DEFAULT);
	}
