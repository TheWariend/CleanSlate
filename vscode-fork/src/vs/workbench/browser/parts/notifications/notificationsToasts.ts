...
import { addDisposableListener, EventType, Dimension, scheduleAtNextAnimationFrame, isAncestorOfActiveElement, getWindow, $, isElementInBottomLeftQuarter, isHTMLElement, isEditableElement, getActiveElement } from '../../../../base/browser/dom.js';
...
export class NotificationsToasts extends Themable implements INotificationsToastController {
	...
	private addToast(item: INotificationViewItem): void {
		...
		if (item.priority === NotificationPriority.OPTIONAL) {
			...
			if (isHTMLElement(activeElement) && isEditableElement(activeElement) && isElementInBottomLeftQuarter(activeElement, this.layoutService.mainContainer)) {
