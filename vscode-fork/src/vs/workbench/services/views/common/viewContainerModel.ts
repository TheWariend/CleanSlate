...
export class ViewContainerModel extends Disposable implements IViewContainerModel {
	...
	private _overriddenTitle: string | undefined;
	...
	private _onDidChangeContainerInfo = this._register(new Emitter<{ title?: boolean; icon?: boolean; keybindingId?: boolean; badgeEnablement?: boolean }>());
	...
	private updateContainerInfo(): void {
		...
		const title = this._overriddenTitle ?? (useDefaultContainerInfo ? (typeof this.viewContainer.title === 'string' ? this.viewContainer.title : this.viewContainer.title.value) : this.visibleViewDescriptors[0]?.containerTitle || this.visibleViewDescriptors[0]?.name?.value || '');
	...
	setTitle(title: string): void {
		if (this._overriddenTitle !== title) {
