	...
	private async shouldBackupBeforeShutdown(reason: ShutdownReason, modifiedWorkingCopies: readonly IWorkingCopy[]): Promise<readonly IWorkingCopy[]> {
		...
		switch (reason) {
			...
			case ShutdownReason.LOAD:
				if (this.contextService.getWorkbenchState() !== WorkbenchState.EMPTY) {
					if (this.filesConfigurationService.hotExitConfiguration !== HotExitConfiguration.OFF) {
						return modifiedWorkingCopies; // preserve unsaved editors when switching projects in the same window
					}
