...
const FALLBACK_DEFAULT_ACCOUNT_AUTH_PROVIDER: IDefaultAccountAuthenticationProvider = {
	id: '',
	name: '',
	enterprise: false
};
...
export class DefaultAccountService extends Disposable implements IDefaultAccountService {
	...
	constructor() {
		super();
		this.defaultAccountConfig = undefined;
	}
	...
	getDefaultAccountAuthenticationProvider(): IDefaultAccountAuthenticationProvider {
		...
		return FALLBACK_DEFAULT_ACCOUNT_AUTH_PROVIDER;
	}
...
class DefaultAccountProvider extends Disposable implements IDefaultAccountProvider {
	...
	private async init(): Promise<void> {
		...
		this._register(this.authenticationService.onDidChangeSessions(e => {
			...
			if (!defaultAccountProvider || e.providerId !== defaultAccountProvider.id) {
				return;
			}
		...
		this._register(this.authenticationService.onDidRegisterAuthenticationProvider(e => {
			const defaultAccountProvider = this.getDefaultAccountAuthenticationProvider();
			if (!defaultAccountProvider || e.id !== defaultAccountProvider.id) {
				return;
			}
	...
	private async fetchDefaultAccount(): Promise<IDefaultAccountData | null> {
		const defaultAccountProvider = this.getDefaultAccountAuthenticationProvider();
		if (!defaultAccountProvider || defaultAccountProvider.id === '') {
			return null;
		}
...
class DefaultAccountProviderContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		super();
		const defaultAccountProvider = this._register(instantiationService.createInstance(DefaultAccountProvider, undefined));
