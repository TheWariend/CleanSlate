...
import { buildCommitMessagePrompt, IDiffSummary } from './scmCleanSlatePrompts.js';
...
const enum SCMInputWidgetCommandId {
	...
	GenerateCommitMessage = 'scm.input.generateCommitMessage',
	CancelAction = 'scm.input.cancel'
}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			menu: [{
				...
				group: 'navigation',
				// when: ChatContextKeys.Setup.installed.negate() -- removed this so it always shows our AI
			}]
		});
	}
	...
	override async run(accessor: ServicesAccessor, ...args: unknown[]): Promise<void> {
		const scmViewService = accessor.get(ISCMViewService);
		const cleanSlateService = accessor.get(ICleanSlateService);
		const fileService = accessor.get(IFileService);

		const repositoryData = scmViewService.activeRepository.get() ?? scmViewService.visibleRepositories[0];

		if (!repositoryData) {
			return;
		}

		const repository = isSCMRepository(repositoryData) ? repositoryData : repositoryData.repository;
		const context = args[1] as ISCMInputValueProviderContext[];

		if (!context || context.length === 0) {
			return;
		}

		// Collect all changed resources
		const resources: URI[] = [];
		...
		const diffSummaries: IDiffSummary[] = [];
		for (const resource of resources) {
			try {
				const originalResource = await repository.provider.getOriginalResource(resource);
				let originalContent = '';
				if (originalResource) {
					try {
						const content = await fileService.readFile(originalResource);
						originalContent = content.value.toString();
					} catch { /* ignore */ }
				}

				const currentContentRaw = await fileService.readFile(resource);
				const currentContent = currentContentRaw.value.toString();
				...
				console.error(`Failed to get diff for ${resource.toString()}`, error);
			}
		}

		const prompt = buildCommitMessagePrompt(diffSummaries);

		try {
			const response = await cleanSlateService.generate(prompt);
			let fullMessage = '';

			for await (const part of response) {
				if (part.type === 'text') {
					fullMessage += part.content;
					repository.input.setValue(fullMessage.trim(), false);
				}
			}
		} catch (error) {
