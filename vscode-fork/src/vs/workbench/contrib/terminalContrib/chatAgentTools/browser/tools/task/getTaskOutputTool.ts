...
export class GetTaskOutputTool extends Disposable implements IToolImpl {
	...
	async prepareToolInvocation(context: IToolInvocationPreparationContext, token: CancellationToken): Promise<IPreparedToolInvocation | undefined> {
		...
		if (!task) {
			return { invocationMessage: new MarkdownString(localize('aiChat.taskNotFound', 'Task not found: \`{0}\`', args.id)) };
		}
		const taskLabel = task._label;
		const activeTasks = await this._tasksService.getActiveTasks();
		if (activeTasks.includes(task)) {
			return { invocationMessage: new MarkdownString(localize('aiChat.taskAlreadyRunning', 'The task \`{0}\` is already running.', taskLabel)) };
		}

		return {
			invocationMessage: new MarkdownString(localize('aiChat.checkingTerminalOutput', 'Checking output for task \`{0}\`', taskLabel)),
			pastTenseMessage: new MarkdownString(localize('aiChat.checkedTerminalOutput', 'Checked output for task \`{0}\`', taskLabel)),
		};
	}
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
		...
		const task = await getTaskForTool(args.id, taskDefinition, args.workspaceFolder, this._configurationService, this._tasksService, true);
		if (!task) {
			return { content: [{ kind: 'text', value: `Task not found: ${args.id}` }], toolResultMessage: new MarkdownString(localize('aiChat.taskNotFound', 'Task not found: \`{0}\`', args.id)) };
		}
		...
		if (!terminals || terminals.length === 0) {
			return { content: [{ kind: 'text', value: `Terminal not found for task ${taskLabel}` }], toolResultMessage: new MarkdownString(localize('aiChat.terminalNotFound', 'Terminal not found for task \`{0}\`', taskLabel)) };
		}
		...
		for (const r of terminalResults) {
			this._telemetryService.publicLog2?.<TaskToolEvent, TaskToolClassification>('aiChat.getTaskOutputTool.get', {
				taskId: args.id,
