...
export class OutputMonitor extends Disposable implements IOutputMonitor {
	...
	private async _selectAndHandleOption(
		...
		const models = await this._languageModelsService.selectLanguageModels({ vendor: 'ai', family: model.replaceAll('ai/', '') });
		if (!models.length) {
			return undefined;
		}
	...
	private async _getLanguageModel(): Promise<string | undefined> {
		let models = await this._languageModelsService.selectLanguageModels({ vendor: 'ai', id: 'ai-fast' });

		// Fallback to gpt-4o-mini if ai-fast is not available for backwards compatibility
		if (!models.length) {
			models = await this._languageModelsService.selectLanguageModels({ vendor: 'ai', family: 'gpt-4o-mini' });
		}
