...
suite('promptFileLocations', function () {
	...
	suite('getPromptFileType', () => {
		...
		test('ai-instructions.md should be recognized as instructions', () => {
			const uri = URI.file('/workspace/.github/ai-instructions.md');
			assert.strictEqual(getPromptFileType(uri), PromptsType.instructions);
		});
	...
	suite('getCleanPromptName', () => {
		...
		test('removes .md extension for ai-instructions.md', () => {
			const uri = URI.file('/workspace/.github/ai-instructions.md');
			assert.strictEqual(getCleanPromptName(uri), 'ai-instructions');
		});
