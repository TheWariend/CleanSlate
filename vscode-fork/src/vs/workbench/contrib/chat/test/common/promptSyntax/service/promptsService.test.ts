...
suite('PromptsService', () => {
	...
	suite('findInstructionFilesFor', () => {
		...
		test('ai-instructions and AGENTS.md', async () => {
			const rootFolderName = 'ai-instructions-and-agents';
			const rootFolder = `/${rootFolderName}`;
			const rootFolderUri = URI.file(rootFolder);
			...
			assert.deepStrictEqual(
				...
				[
					URI.joinPath(rootFolderUri, '.github/ai-instructions.md').path,
			...
			await mockFiles(fileService, [
				{
					...
					contents: [
						'---',
						'description: \'CleanSlate AI specialized agent.\'',
						'target: \'cleanslate-ai\'',
						'tools: [ github-api, code-search ]',
						'---',
						'I am optimized for CleanSlate AI workflows.',
					]
				},
				{
			...
			const expected: ICustomAgent[] = [
				{
					...
					tools: ['github-api', 'code-search'],
					agentInstructions: {
						content: 'I am optimized for CleanSlate AI workflows.',
						toolReferences: [],
						metadata: undefined
					},
					handOffs: undefined,
	...
	suite('listPromptFiles - skills ', () => {
		...
		test('should list skill files from user home', async () => {
			...
			const aiSkill = personalSkills.find(s => s.uri.path.includes('.ai'));
			assert.ok(aiSkill, 'Should find ai personal skill');
	...
	suite('findAgentSkills', () => {
		...
		test('should find skills in workspace and user home', async () => {
			...
			await mockFiles(fileService, [
				...
				{
					path: '/home/user/.ai/skills/AI Skill 1/SKILL.md',
					contents: [
						'---',
						'name: "AI Skill 1"',
						'description: "A AI skill for testing"',
						'---',
						'This is AI skill 1 content',
					],
				},
			]);
			...
			const aiSkill1 = personalSkills.find(skill => skill.name === 'AI Skill 1');
			assert.ok(aiSkill1, 'Should find AI Skill 1');
			assert.strictEqual(aiSkill1.description, 'A AI skill for testing');
			assert.strictEqual(aiSkill1.uri.path, '/home/user/.ai/skills/AI Skill 1/SKILL.md');
		});
		...
		test('should skip duplicate skill names and keep first by priority', async () => {
			...
			await mockFiles(fileService, [
				...
				{
					path: '/home/user/.ai/skills/Duplicate Skill/SKILL.md',
					contents: [
						'---',
						'name: "Duplicate Skill"',
		...
		test('should prioritize skills by source: workspace > user > extension', async () => {
			...
			await mockFiles(fileService, [
				{
					path: '/home/user/.ai/skills/Priority Skill/SKILL.md',
					contents: [
						'---',
						'name: "Priority Skill"',
