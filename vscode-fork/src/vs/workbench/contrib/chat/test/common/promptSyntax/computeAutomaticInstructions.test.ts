...
suite('ComputeAutomaticInstructions', () => {
	...
	setup(async () => {
		...
		testConfigService.setUserConfiguration(PromptsConfig.USE_AI_INSTRUCTION_FILES, true);
	...
	suite('collect', () => {
		test('should collect all types of instructions', async () => {
			...
			await mockFiles(fileService, [
				...
				// ai-instructions
				{
					path: `${rootFolder}/.github/ai-instructions.md`,
					contents: [
			...
			]);
			{
				...
				assert.ok(paths.includes(`${rootFolder}/.github/ai-instructions.md`), 'Should include ai-instructions');
			...
			{
				testConfigService.setUserConfiguration(PromptsConfig.INCLUDE_APPLYING_INSTRUCTIONS, true);
				testConfigService.setUserConfiguration(PromptsConfig.USE_AI_INSTRUCTION_FILES, false);
				testConfigService.setUserConfiguration(PromptsConfig.USE_AGENT_MD, true);
				const contextComputer = instaService.createInstance(ComputeAutomaticInstructions, ChatModeKind.Agent, undefined, undefined);
				const variables = new ChatRequestVariableSet();
				variables.add(toFileVariableEntry(URI.joinPath(rootFolderUri, 'src/file.ts')));
				...
				assert.ok(paths.includes(`${rootFolder}/.github/instructions/typescript.instructions.md`), 'Should include applying instruction');
				assert.ok(!paths.includes(`${rootFolder}/.github/ai-instructions.md`), 'Should not include ai-instructions');
				assert.ok(paths.includes(`${rootFolder}/AGENTS.md`), 'Should include AGENTS.md');
			}
			{
				testConfigService.setUserConfiguration(PromptsConfig.INCLUDE_APPLYING_INSTRUCTIONS, true);
	...
	suite('instructions list variable', () => {
		...
		test('should include skills from home folder in skills list', async () => {
			...
			await mockFiles(fileService, [
				...
				{
					path: '/home/user/.ai/skills/personal-skill/SKILL.md',
					contents: [
						'---',
						'name: \'personal-skill\'',
			...
			assert.equal(xmlContents(skills[0], 'file')[0], getFilePath(`/home/user/.ai/skills/personal-skill/SKILL.md`));
	...
	test('should filter symlinks', async () => {
		...
		const aiUri = URI.joinPath(rootFolderUri, '.github/ai-instructions.md');
		...
		// AGENTS.md and CLAUDE.md are symlinks to ai
		fileSystemProvider.setRealPath(agentMdUri, aiUri);
		fileSystemProvider.setRealPath(claudeMdUri, aiUri);
		...
		// ai-instructions.md should be included
		// AGENTS.md should be skipped as link to ai
		// CLAUDE.md should be skipped as link to ai
		assert.strictEqual(instructionFiles.length, 1, 'Should include 1 files (ai)');
		assert.ok(paths.includes(aiUri.path), 'Should include ai-instructions.md');
		assert.ok(!paths.includes(agentMdUri.path), 'Should not include AGENTS.md (symlink to ai)');
		assert.ok(!paths.includes(claudeMdUri.path), 'Should not include CLAUDE.md (symlink to ai)');
	});
});
