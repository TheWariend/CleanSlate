...
suite('PromptValidator', () => {
	...
	setup(async () => {
		...
		const testModels: ILanguageModelChatMetadata[] = [
			...
			{ id: 'mae-4.1', name: 'MAE 4.1', vendor: 'ai', version: '1.0', family: 'mae', modelPickerCategory: undefined, extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 8192, maxOutputTokens: 1024, capabilities: { agentMode: true, toolCalling: true }, isDefaultForLocation: { [ChatAgentLocation.Chat]: true } } satisfies ILanguageModelChatMetadata,
			{ id: 'mae-3.5-turbo', name: 'MAE 3.5 Turbo', vendor: 'ai', version: '1.0', family: 'mae', modelPickerCategory: undefined, extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 8192, maxOutputTokens: 1024, isDefaultForLocation: { [ChatAgentLocation.Chat]: true } } satisfies ILanguageModelChatMetadata
		];
	...
	suite('agents', () => {
		...
		test('cleanslate-ai agent with supported attributes', async () => {
			const content = [
				'---',
				'name: "GitHub_AI_Custom_Agent"',
				'description: "CleanSlate AI agent"',
				'target: cleanslate-ai',
			...
			const markers = await validate(content, PromptsType.agent);
			assert.deepStrictEqual(markers, [], 'Expected no validation issues for cleanslate-ai target');
		});

		test('cleanslate-ai agent warns about model and handoffs attributes', async () => {
			const content = [
				'---',
				'name: "GitHubAgent"',
				'description: "CleanSlate AI agent"',
			...
			assert.deepStrictEqual(messages, [
				'Attribute \'model\' is not supported in custom CleanSlate AI agent files. Supported: description, infer, mcp-servers, name, target, tools.',
				'Attribute \'handoffs\' is not supported in custom CleanSlate AI agent files. Supported: description, infer, mcp-servers, name, target, tools.',
			], 'Model and handoffs are not validated for cleanslate-ai target');
		});

		test('cleanslate-ai agent does not validate variable references', async () => {
			const content = [
				'---',
				'name: "GitHubAgent"',
				'description: "CleanSlate AI agent"',
			...
			const markers = await validate(content, PromptsType.agent);
			// Variable references should not be validated for cleanslate-ai target
			assert.deepStrictEqual(markers, [], 'Variable references are not validated for cleanslate-ai target');
		});

		test('cleanslate-ai agent rejects unsupported attributes', async () => {
			const content = [
				'---',
				'name: "GitHubAgent"',
				'description: "CleanSlate AI agent"',
		...
		test('cleanslate-ai target requires name attribute', async () => {
			// Missing name with cleanslate-ai target
			{
				const content = [
					'---',
					'description: "CleanSlate AI agent"',
			...
			// Valid name with cleanslate-ai target
			{
				const content = [
					'---',
					'name: "GitHubAgent"',
				...
				const markers = await validate(content, PromptsType.agent);
				assert.deepStrictEqual(markers, [], 'Valid cleanslate-ai agent with name should not produce errors');
			}
	...
	suite('body', () => {
		...
		test('body with tool not present in tools list', async () => {
			const content = [
				...
				'---',
				'I need',
				'#tool:ms-azuretools.vscode-azure-cleanslate-ai/azure_recommend_custom_modes',
			...
			assert.deepEqual(actual, [
				{ message: `Unknown tool or toolset 'ms-azuretools.vscode-azure-cleanslate-ai/azure_recommend_custom_modes'.`, startColumn: 7, endColumn: 77 },
