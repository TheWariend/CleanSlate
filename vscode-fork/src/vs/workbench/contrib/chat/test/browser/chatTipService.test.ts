...
suite('ChatTipService', () => {
	...
	function createService(tipsEnabled: boolean = true): ChatTipService {
		instantiationService.stub(IProductService, createProductService());
	...
	// test('returns undefined when AI is not enabled', () => {
	// 	const service = createService(/* hasAI */ false);
	// 	const now = Date.now();
	//
	// 	const tip = service.getNextTip('request-1', now + 1000, contextKeyService);
	// 	assert.strictEqual(tip, undefined, 'Should not return a tip when AI is not enabled');
	// });

	test('returns undefined when tips setting is disabled', () => {
		const service = createService(/* tipsEnabled */ false);
		const now = Date.now();
