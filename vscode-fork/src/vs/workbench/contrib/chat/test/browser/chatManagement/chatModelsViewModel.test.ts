...
class MockLanguageModelsService implements ILanguageModelsService {
	...
	getVendors(): ILanguageModelProviderDescriptor[] {
		return this.vendors.map(v => ({ ...v, isDefault: v.vendor === 'ai' }));
	}
...
suite('ChatModelsViewModel', () => {
	...
	setup(async () => {
		...
		languageModelsService.addModel('ai', 'ai-gpt-4', {
			extension: new ExtensionIdentifier('github.ai'),
			id: 'gpt-4',
			name: 'GPT-4',
			family: 'gpt-4',
			version: '1.0',
			...
			modelPickerCategory: { label: 'AI', order: 1 },
		...
		languageModelsService.addModel('ai', 'ai-gpt-4o', {
			extension: new ExtensionIdentifier('github.ai'),
			id: 'gpt-4o',
			name: 'GPT-4o',
			family: 'gpt-4',
	...
	test('should filter by provider name (vendor ID and display name)', () => {
		const resultsByAIId = viewModel.filter('@provider:ai');
		assert.strictEqual(resultsByAIId.length, 3);
		assert.strictEqual(resultsByAIId[0].type, 'vendor');
		assert.strictEqual(resultsByAIId[0].vendorEntry.vendor.vendor, 'ai');
		assert.strictEqual(resultsByAIId[1].type, 'model');
		assert.strictEqual(resultsByAIId[1].model.identifier, 'ai-gpt-4');
		assert.strictEqual(resultsByAIId[2].type, 'model');
		assert.strictEqual(resultsByAIId[2].model.identifier, 'ai-gpt-4o');
	...
	test('should filter by multiple providers with OR logic', () => {
		const results = viewModel.filter('@provider:ai @provider:openai');
	...
	test('should combine provider and capability filters', () => {
		const results = viewModel.filter('@provider:ai @capability:vision');

		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		assert.strictEqual(models.length, 2);
		assert.ok(models.every(m =>
			m.model.provider.vendor.vendor === 'ai' &&
			m.model.metadata.capabilities?.vision === true
		));
	});
	...
	test('should filter by text matching model id', () => {
		...
		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		assert.strictEqual(models.length, 1);
		assert.strictEqual(models[0].model.identifier, 'ai-gpt-4o');
	...
	test('should filter by text matching vendor name', () => {
		...
		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		assert.strictEqual(models.length, 2);
		assert.ok(models.every(m => m.model.provider.group.name === 'CleanSlate AI'));
	});
	...
	test('should toggle vendor collapsed state', () => {
		const vendorEntry = viewModel.viewModelEntries.find(r => isLanguageModelProviderEntry(r) && r.vendorEntry.vendor.vendor === 'ai') as ILanguageModelProviderEntry;
		viewModel.toggleCollapsed(vendorEntry);

		const results = viewModel.filter('');
		const aiVendor = results.find(r => isLanguageModelProviderEntry(r) && (r as ILanguageModelProviderEntry).vendorEntry.vendor.vendor === 'ai') as ILanguageModelProviderEntry;
		assert.ok(aiVendor);
		assert.strictEqual(aiVendor.collapsed, true);

		// Models should not be shown when vendor is collapsed
		const aiModelsAfterCollapse = results.filter(r =>
			!isLanguageModelProviderEntry(r) && (r as ILanguageModelEntry).model.provider.vendor.vendor === 'ai'
		);
		assert.strictEqual(aiModelsAfterCollapse.length, 0);

		// Toggle back
		viewModel.toggleCollapsed(vendorEntry);
		const resultsAfterExpand = viewModel.filter('');
		const aiModelsAfterExpand = resultsAfterExpand.filter(r =>
			!isLanguageModelProviderEntry(r) && (r as ILanguageModelEntry).model.provider.vendor.vendor === 'ai'
		);
		assert.strictEqual(aiModelsAfterExpand.length, 2);
	});
	...
	test('should remove filter keywords from text search', () => {
		const results = viewModel.filter('@provider:ai @capability:vision GPT');

		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		// Should only search 'GPT' in model names, not the filter keywords
		assert.strictEqual(models.length, 2);
		assert.ok(models.every(m => m.model.provider.vendor.vendor === 'ai'));
	});
	...
	test('should always place ai vendor at the top when multiple vendors exist', async () => {
		// Test with default setup (ai and openai)
		let results = viewModel.filter('');
		let vendors = results.filter(isLanguageModelProviderEntry) as ILanguageModelProviderEntry[];
		assert.strictEqual(vendors[0].vendorEntry.vendor.vendor, 'ai');
	...
	test('should group by visibility', () => {
		...
		assert.strictEqual(actuals[1].model.identifier, 'ai-gpt-4');
		assert.strictEqual(actuals[2].type, 'model');
		assert.strictEqual(actuals[2].model.identifier, 'ai-gpt-4o');
	...
	test('should get configured vendors', () => {
		...
		assert.ok(vendors.length > 0);
		assert.ok(vendors.some(v => v.vendor.vendor === 'ai'));
	...
	test('should filter by case-insensitive provider name', () => {
		const results1 = viewModel.filter('@provider:AI');
		const results2 = viewModel.filter('@provider:ai');
		const results3 = viewModel.filter('@provider:AI');
