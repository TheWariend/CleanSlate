...
suite('formatStatusOutput', () => {
	...
	const emptySpecialFiles = {
		...
		aiInstructions: { enabled: false, files: [] },
	...
	test('agents with overwritten files', () => {
		const statusInfos: ITypeStatusInfo[] = [{
			...
			paths: [
				...
				createPath('~/.ai/agents', true, PromptsStorage.user)
			],
			files: [
				createFile('my-agent.agent.md', 'loaded', '.github/agents'),
				createFile('my-agent.agent.md', 'overwritten', 'home/.ai/agents', PromptsStorage.user, { overwrittenBy: 'my-agent.agent.md' })
			],
			enabled: true
		}];
		...
		assert.strictEqual(output, lines(
			...
			'~/.ai/agents<br>',
			`${TREE_END} ${ICON_WARN} [\`my-agent.agent.md\`](${filePath('home/.ai/agents/my-agent.agent.md')}) - *Overwritten by higher priority file*<br>`,
			''
		));
	});
	...
	test('instructions with ai-instructions.md enabled', () => {
		const statusInfos: ITypeStatusInfo[] = [{
		...
		const specialFiles = {
			agentsMd: { enabled: false, files: [] },
			aiInstructions: { enabled: true, files: [URI.file('/workspace/.github/ai-instructions.md')] },
			claudeMd: { enabled: false, files: [] }
		};
		...
		assert.strictEqual(output, lines(
			...
			'AGENTS.md -<br>',
			'ai-instructions.md<br>',
			`${TREE_END} [\`ai-instructions.md\`](${filePath('.github/ai-instructions.md')})<br>`,
			''
		));
	});
