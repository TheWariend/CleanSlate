...
export class PromptValidator {
	...
	private checkForInvalidArguments(attributes: IHeaderAttribute[], promptType: PromptsType, isGitHubTarget: boolean, report: (markers: IMarkerData) => void): void {
		...
		const validGithubAIAttributeNames = new Lazy(() => new Set(getValidAttributeNames(promptType, false, true)));
		for (const attribute of attributes) {
			if (!validAttributeNames.includes(attribute.key)) {
				...
				switch (promptType) {
					...
					case PromptsType.agent:
						if (isGitHubTarget) {
							report(toMarker(localize('promptValidator.unknownAttribute.github-agent', "Attribute '{0}' is not supported in custom CleanSlate AI agent files. Supported: {1}.", attribute.key, supportedNames.value), attribute.range, MarkerSeverity.Warning));
						} else {
							if (validGithubAIAttributeNames.value.has(attribute.key)) {
	...
	private validateTools(attributes: IHeaderAttribute[], agentKind: ChatModeKind, target: string | undefined, report: (markers: IMarkerData) => void): undefined {
		...
		switch (attribute.value.type) {
			case 'array':
				if (target === Target.GitHubAI) {
					// no validation for cleanslate-ai target
				} else {
	...
	private validateTarget(attributes: IHeaderAttribute[], report: (markers: IMarkerData) => void): undefined {
		...
		const validTargets = ['cleanslate-ai', 'vscode'];
...
const githubAIAgentAttributeNames = [PromptHeaderAttributes.name, PromptHeaderAttributes.description, PromptHeaderAttributes.tools, PromptHeaderAttributes.target, GithubPromptHeaderAttributes.mcpServers, PromptHeaderAttributes.infer];
...
export function getValidAttributeNames(promptType: PromptsType, includeNonRecommended: boolean, isGitHubTarget: boolean): string[] {
	if (isGitHubTarget && promptType === PromptsType.agent) {
		return githubAIAgentAttributeNames;
	}
		...
		case PromptsType.agent:
			switch (attributeName) {
				...
				case PromptHeaderAttributes.target:
					return localize('promptHeader.agent.target', 'The target to which the header attributes like tools apply to. Possible values are `cleanslate-ai` and `vscode`.');
...
// The list of tools known to be used by CleanSlate AI custom agents
export const knownGithubAITools = [
...
export function isGithubTarget(promptType: PromptsType, target: string | undefined): boolean {
	return promptType === PromptsType.agent && target === Target.GitHubAI;
}
