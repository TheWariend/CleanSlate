...
/**
 * AI custom instructions file name.
 */
export const AI_CUSTOM_INSTRUCTIONS_FILENAME = 'ai-instructions.md';
...
export interface IResolvedPromptSourceFolder {
	...
	readonly storage: PromptsStorage;
	/**
	 * The original path string before resolution (e.g., '~/.ai/agents' or '.github/agents').
...
export const DEFAULT_SKILL_SOURCE_FOLDERS: readonly IPromptSourceFolder[] = [
	...
	{ path: '~/.ai/skills', source: PromptFileSource.AIPersonal, storage: PromptsStorage.user },
...
export function getPromptFileType(fileUri: URI): PromptsType | undefined {
	...
	if (filename.endsWith(INSTRUCTION_FILE_EXTENSION) || (filename === AI_CUSTOM_INSTRUCTIONS_FILENAME)) {
...
export function getCleanPromptName(fileUri: URI): string {
	...
	if (fileName === AI_CUSTOM_INSTRUCTIONS_FILENAME) {
