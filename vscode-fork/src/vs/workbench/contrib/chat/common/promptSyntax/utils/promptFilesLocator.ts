...
import { AI_CUSTOM_INSTRUCTIONS_FILENAME, AGENTS_SOURCE_FOLDER, getPromptFileExtension, getPromptFileType, LEGACY_MODE_FILE_EXTENSION, getCleanPromptName, AGENT_FILE_EXTENSION, getPromptFileDefaultLocations, SKILL_FILENAME, IPromptSourceFolder, DEFAULT_AGENT_SOURCE_FOLDERS, DEFAULT_HOOK_FILE_PATHS, IResolvedPromptFile, IResolvedPromptSourceFolder, PromptFileSource, HOOKS_SOURCE_FOLDER } from '../config/promptFileLocations.js';
...
export class PromptFilesLocator {
		...
		if (type !== PromptsType.skill) {
			...
			return [
				...result,
				{
					...
					source: PromptFileSource.AIPersonal,
	 ...
	 * Returns only the AI hooks folder (.github/hooks) since Claude paths are read-only.
	 */
	...
	public async findAIInstructionsMDsInWorkspace(token: CancellationToken): Promise<IResolvedAgentFile[]> {
		const result: IResolvedAgentFile[] = [];
		const { folders } = this.workspaceService.getWorkspace();
		for (const folder of folders) {
			const file = joinPath(folder.uri, `.github/` + AI_CUSTOM_INSTRUCTIONS_FILENAME);
			try {
				...
				if (stat.isFile) {
					...
					result.push({ uri: file, realPath, type: AgentFileType.aiInstructionsMd });
				}
			} catch (error) {
				this.logService.trace(`[PromptFilesLocator] Skipping ai-instructions.md at ${file.toString()}: ${error}`);
			}
		}
		return result;
	}
