...
export class PromptsService extends Disposable implements IPromptsService {
	...
	public async getSourceFolders(type: PromptsType): Promise<readonly IPromptPath[]> {
		...
		if (type === PromptsType.agent) {
			...
			// For hooks, return the AI hooks folder for creating new hooks
	...
	public async listAIInstructionsMDs(token: CancellationToken, logger: Logger | undefined): Promise<IResolvedAgentFile[]> {
		const useAIInstructionsFiles = this.configurationService.getValue(PromptsConfig.USE_AI_INSTRUCTION_FILES);
		if (!useAIInstructionsFiles) {
			logger?.logInfo('AI instructions files are disabled via configuration.');
			return [];
		}
		return await this.fileLocator.findAIInstructionsMDsInWorkspace(token);
	}

	public async listAgentInstructions(token: CancellationToken, logger: Logger | undefined): Promise<IResolvedAgentFile[]> {
		const [agentMDs, claudeMDs, aiInstructionsMDs] = await Promise.all([
			this.listAgentMDs(token, logger),
			this.listClaudeMDs(token, logger),
			this.listAIInstructionsMDs(token, logger)
		]);
		if (token.isCancellationRequested) {
			return [];
		}
	...
	private async computeAgentSkills(token: CancellationToken): Promise<IAgentSkill[]> {
		...
		type AgentSkillsFoundClassification = {
			...
			aiPersonal: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; isMeasurement: true; comment: 'Number of AI personal skills.' };
		...
		this.telemetryService.publicLog2<AgentSkillsFoundEvent, AgentSkillsFoundClassification>('agentSkillsFound', {
			...
			aiPersonal: skillsBySource.get(PromptFileSource.AIPersonal) ?? 0,
	...
	private async computeHooks(token: CancellationToken): Promise<IChatRequestHooks | undefined> {
		...
		for (const hookFile of hookFiles) {
			try {
				...
				// Use format-aware parsing that handles AI, Claude, and Cursor formats
	...
	private async getHookDiscoveryInfo(token: CancellationToken): Promise<IPromptDiscoveryInfo> {
		...
		for (const promptPath of hookFiles) {
			...
			try {
				...
				// Validate version field for AI hooks.json format
