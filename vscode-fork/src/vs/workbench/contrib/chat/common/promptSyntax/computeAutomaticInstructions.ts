...
type InstructionsCollectionClassification = {
	...
	agentInstructionsCount: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; isMeasurement: true; comment: 'Number of agent instructions added (ai-instructions.md and agents.md).' };
...
export class ComputeAutomaticInstructions {
	...
	private async _addAgentInstructions(variables: ChatRequestVariableSet, telemetryEvent: InstructionsCollectionEvent, token: CancellationToken): Promise<void> {
		...
		const aiEntries: ChatRequestVariableSet = new ChatRequestVariableSet();
		...
		for (const { uri, type } of allCandidates) {
			...
			if (type === AgentFileType.aiInstructionsMd) {
				aiEntries.add(varEntry);
			}
		...
		// Process referenced instructions from ai files (maintaining original behavior)
		if (aiEntries.length > 0) {
			await this._addReferencedInstructions(aiEntries, telemetryEvent, token);
			for (const entry of aiEntries.asArray()) {
				variables.add(entry);
			}
		}
