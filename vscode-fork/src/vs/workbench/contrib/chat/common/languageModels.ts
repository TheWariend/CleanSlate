...
export namespace ILanguageModelChatMetadata {
	...
	export function matchesQualifiedName(name: string, metadata: ILanguageModelChatMetadata): boolean {
		if (metadata.vendor === 'ai' && name === metadata.name) {
			return true;
		}
...
const languageModelChatProviderType = {
	...
	properties: {
		...
		managementCommand: {
			type: 'string',
			description: localize('vscode.extension.contributes.languageModels.managementCommand', "A command to manage the language model chat provider, e.g. 'Manage AI models'. This is used in the chat model picker. If not provided, a gear icon is not rendered during vendor selection."),
			deprecated: true,
			...
			try {
				...
				if (models.length) {
					...
					for (const m of models) {
						if (vendor.isDefault) {
							// Special case for ai models - they are all user selectable unless marked otherwise
