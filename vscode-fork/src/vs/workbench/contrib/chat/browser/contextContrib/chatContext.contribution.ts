			...
			for (const ext of extensions) {
				...
				for (const contribution of ext.value) {
					...
					if (!icon && contribution.icon) {
						ext.collector.error(localize('chatContextExtPoint.invalidIcon', "Invalid icon format for chat context contribution '{0}'. Icon must be in the format '$(iconId)' or '$(iconId~spin)', e.g. '$(ai)'.", contribution.id));
						continue;
					}
					if (!icon) {
