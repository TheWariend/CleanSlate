...
export function registerNewChatActions() {
	registerAction2(class NewChatAction extends Action2 {
			...
			super({
				...
				menu: [
					...
					},
					{
						...
						group: 'navigation',
						when: ContextKeyExpr.and(ChatContextKeys.location.isEqualTo(ChatAgentLocation.Chat), ChatContextKeys.agentSessionsViewerOrientation.notEqualsTo(AgentSessionsViewerOrientation.SideBySide)),
						order: 1
					}
				],
				keybinding: {
		...
		async run(accessor: ServicesAccessor) {
			...
			await commandService.executeCommand('cleanSlate.chat.newChat');
		}
	});
