...
export class PromptFilePickers {
	...
	private async _createPromptPickItems(options: ISelectOptions, token: CancellationToken): Promise<(IPromptPickerQuickPickItem | IQuickPickSeparator)[]> {
		...
		// Agent instruction files (ai-instructions.md and AGENTS.md) are added here and not included in the output of
		...
		if (options.type === PromptsType.instructions) {
			...
			agentInstructionFiles = agentInstructionUris.map(agentInstructionFile => {
				...
				// Don't show the folder path for files under .github folder (namely, ai-instructions.md) since that is only defined once per repo.
				return {
					uri: agentInstructionFile.uri,
					description: agentInstructionFile.type !== AgentFileType.aiInstructionsMd ? folderName : undefined,
