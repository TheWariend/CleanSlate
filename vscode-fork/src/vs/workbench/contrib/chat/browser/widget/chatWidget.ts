...
export class ChatWidget extends Disposable implements IChatWidget {
		...
		try {
			...
			if (!numItems) {
				...
				if (this.chatEntitlementService.anonymous && !this.chatEntitlementService.sentiment.installed) {
					additionalMessage = new MarkdownString(localize({ key: 'settings', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing, you agree to the [Terms]({0}) and [Privacy Statement]({1}).", 'https://github.com/site-policy/github-terms/github-event-terms-of-service', 'https://github.com/site-policy/privacy-policies/github-privacy-statement'), { isTrusted: true });
				} else {
	...
	/**
	 * Checks if any agent instruction files (.github/ai-instructions.md or AGENTS.md) exist in the workspace.
	...
	private getWelcomeViewContent(additionalMessage: string | IMarkdownString | undefined): IChatViewWelcomeContent {
		if (this.isLockedToCodingAgent) {
			...
			const message = providerMessage
				...
				: (this._lockedAgent?.prefix === '@ai '
					? new MarkdownString(localize('aiCodingAgentMessage', "This chat session will be forwarded to the {0} [coding agent]({1}) where work is completed in the background. ", this._lockedAgent.prefix, 'https://aka.ms/coding-agent-docs') + DISCLAIMER, { isTrusted: true })
	 ...
	 * - instructions referenced in the settings 'ai-instructions'
