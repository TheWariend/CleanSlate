...
export class ChatThinkingContentPart extends ChatCollapsibleContentPart implements IChatContentPart {
	...
	private async generateTitleViaLLM(): Promise<void> {
		...
		try {
			let models = await this.languageModelsService.selectLanguageModels({ vendor: 'openai', id: 'gpt-4o' });
			if (!models.length) {
				models = await this.languageModelsService.selectLanguageModels({ family: 'gpt-4o-mini' });
			}
			if (!models.length) {
				this.setFallbackTitle();
				return;
