...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	private initSelectedModel() {
		...
		const persistedAsDefault = this.storageService.getBoolean(this.getSelectedModelIsDefaultStorageKey(), StorageScope.APPLICATION, persistedSelection === 'openai/gpt-4o');
	...
	public switchModelByQualifiedName(qualifiedModelNames: readonly string[]): boolean {
		...
		this.logService.warn(`[chat] None of the models "${qualifiedModelNames.join(', ')}" found. Use format "<name> (<vendor>)", e.g. "GPT-4o (openai)".`);
		return false;
	}
