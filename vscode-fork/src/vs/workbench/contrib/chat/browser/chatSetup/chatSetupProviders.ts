...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	private static readonly SETUP_NEEDED_MESSAGE = new MarkdownString(localize('settingUpAiNeeded', "You need to be signed in to use Chat."));
	...
	private async forwardRequestToChat(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		try {
			...
			progress({
				kind: 'warning',
				content: new MarkdownString(localize('chatUnavailableWarning', "Failed to get a response. Please try again."))
			});
		}
	}
	...
	private replaceAgentInRequestModel(requestModel: IChatRequestModel, chatAgentService: IChatAgentService): IChatRequestModel {
		...
		const chatAgent = chatAgentService.getAgent(agentId);
		if (!chatAgent) {
			return requestModel;
		}

		const newAgentPart = new ChatRequestAgentPart(agentPart.range, agentPart.editorRange, chatAgent);
	...
	private replaceToolInRequestModel(requestModel: IChatRequestModel): IChatRequestModel {
		...
		const toolId = toolPart.toolId.replace(/setup.tools\./, `chat_`.toLowerCase());
...
export class ChatCodeActionsProvider {
	...
	async provideCodeActions(model: ITextModel, range: Range | Selection): Promise<CodeActionList | undefined> {
		...
		if (generateOrModifyTitle && generateOrModifyCommand) {
			actions.push({
				kind: CodeActionKind.RefactorRewrite.append('chat').value,
				isAI: true,
		...
		if (markers.length > 0) {
			...
			actions.push({
				kind: CodeActionKind.QuickFix.append('chat').value,
				isAI: true,
				diagnostics: markers,
			...
			actions.push({
				kind: CodeActionKind.QuickFix.append('explain').append('chat').value,
				isAI: true,
				diagnostics: markers,
