...
const defaultChat = {
	publicCodeMatchesUrl: 'https://code.visualstudio.com/docs/editor/artificial-intelligence',
	provider: { default: { id: 'ai', name: 'AI' }, enterprise: { id: 'ai-enterprise', name: 'AI Enterprise' }, apple: { id: 'apple', name: 'Apple' }, google: { id: 'google', name: 'Google' } },
	manageSettingsUrl: 'https://code.visualstudio.com/docs/editor/artificial-intelligence',
	completionsRefreshTokenCommand: 'ai.chat.refreshToken',
	chatRefreshTokenCommand: 'ai.chat.refreshToken',
	termsStatementUrl: 'https://github.com/site-policy/github-terms/github-event-terms-of-service',
	privacyStatementUrl: 'https://github.com/site-policy/privacy-policies/github-privacy-statement'
};
...
export class ChatSetup {
	...
	static getInstance(instantiationService: IInstantiationService, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): ChatSetup {
		...
		if (!instance) {
			instance = ChatSetup.instance = instantiationService.invokeFunction(accessor => {
				return new ChatSetup(context, controller, accessor.get(ITelemetryService), accessor.get(IWorkbenchLayoutService), accessor.get(IKeybindingService), accessor.get(IChatEntitlementService) as ChatEntitlementService, accessor.get(ILogService), accessor.get(IChatWidgetService), accessor.get(IWorkspaceTrustRequestService), accessor.get(IMarkdownRendererService), accessor.get(IDefaultAccountService));
			});
		}
	...
	private createDialogFooter(disposables: DisposableStore, options?: { forceAnonymous?: ChatSetupAnonymous }): HTMLElement {
		...
		if (options?.forceAnonymous || this.telemetryService.telemetryLevel === TelemetryLevel.NONE) {
			...
			footer = localize({ key: 'settings', comment: ['{Locked="["}', '{Locked="]({1})"}', '{Locked="]({2})"}', '{Locked="]({4})"}', '{Locked="]({5})"}'] }, "By continuing, you agree to {0}'s [Terms]({1}) and [Privacy Statement]({2}). {3} may show [public code]({4}) suggestions and use your data to improve the product. You can change these [settings]({5}) anytime.", defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl, defaultChat.provider.default.name, defaultChat.publicCodeMatchesUrl, defaultChat.manageSettingsUrl);
		}
