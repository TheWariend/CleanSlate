...
import { registerChatLanguageModelActions } from './actions/chatLanguageModelActions.js';
import { registerChatMoveActions } from './actions/chatMoveActions.js';
...
import { registerChatEditingEditorActions } from './chatEditing/chatEditingEditorActions.js';
	...
	properties: {
		...
		[PromptsConfig.HOOKS_LOCATION_KEY]: {
			...
			markdownDescription: nls.localize(
				...
				"Specify paths to hook configuration files that define custom shell commands to execute at strategic points in an agent's workflow. [Learn More]({0}).\n\nRelative paths are resolved from the root folder(s) of your workspace. Supports AI hooks (`hooks.json`) and Claude Code hooks (`settings.json`, `settings.local.json`).",
		...
		[ChatConfiguration.AIDisabled]: {
			type: 'boolean',
			description: nls.localize('chat.disableAIFeatures', "Disable and hide built-in AI features provided by CleanSlate AI, including chat and inline suggestions."),
			default: false,
		...
		'chat.extensionUnification.enabled': {
			type: 'boolean',
			description: nls.localize('chat.extensionUnification.enabled', "Enables the unification of CleanSlate AI extensions. When enabled, all CleanSlate AI functionality is served from the CleanSlate AI Chat extension. When disabled, the CleanSlate AI and CleanSlate AI Chat extensions operate independently."),
			default: true,
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
...
registerChatMoveActions();
registerNewChatActions();
registerChatContextActions();
registerChatDeveloperActions();
registerChatEditingEditorActions();
registerChatElicitationActions();
registerChatToolActions();
registerChatLanguageModelActions();
