...
export class ChatEditingExplanationModelManager extends Disposable implements IChatEditingExplanationModelManager {
	...
	private async _doGenerateExplanations(diffInfos: readonly IExplanationDiffInfo[], cancellationToken: CancellationToken): Promise<void> {
		...
		try {
			...
			let models = await this._languageModelsService.selectLanguageModels({ vendor: 'openai', family: 'gpt-4o' });
			if (!models.length) {
				models = await this._languageModelsService.selectLanguageModels({ family: 'claude-3.5-sonnet' });
			}
			if (!models.length) {
				models = await this._languageModelsService.selectLanguageModels({ family: 'gpt-4o' });
			}
			if (!models.length) {
				models = await this._languageModelsService.selectLanguageModels({ family: 'gpt-4' });
			}
			if (!models.length) {
