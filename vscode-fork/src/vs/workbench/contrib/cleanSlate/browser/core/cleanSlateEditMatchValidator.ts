...
export class CleanSlateEditMatchValidator {
	...
	private static contextMatches(model: ITextModel, anchorLine: number, direction: 'before' | 'after', contextText: string | undefined): boolean {
		if (!contextText?.trim()) {
			return false;
		}
	...
	private static hasStructuralAlignment(model: ITextModel, range: Range, searchText: string): boolean {
