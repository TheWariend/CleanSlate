...
export class CleanSlateFileHistory {
	...
	private static async restoreContent(
		...
		const model: ITextModel | null | undefined = modelService?.getModel(resource);
		if (model) {
	...
	private static async readManifest(fileService: IFileService, historyRoot: URI): Promise<CleanSlateFileHistoryManifest> {
