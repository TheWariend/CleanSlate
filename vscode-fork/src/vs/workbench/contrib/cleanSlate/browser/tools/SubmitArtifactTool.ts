...
import { CLEANSLATE_ARTIFACT_SCHEME } from '../../../../services/cleanSlate/common/core/cleanSlateAI.js';
...
async function openArtifactEditor(context: CleanSlateToolContext, artifactId: string, filename: string, preserveFocus: boolean): Promise<void> {
    ...
    try {
        const artifactInput = context.instantiationService.createInstance(CleanSlateArtifactInput, virtualUri, artifactId);
        await context.editorService.openEditor(artifactInput, { pinned: true, preserveFocus }, ACTIVE_GROUP);
    } catch (e) {
        console.error('[SubmitArtifactTool] Failed to open artifact editor:', e);
    }
}
