...
const enum AddConfigurationAICommand {
	...
	IsSupported = 'github.ai.chat.mcp.setup.check',

	/** Takes an npm/pip package name, validates its owner. */
	ValidatePackage = 'github.ai.chat.mcp.setup.validatePackage',

	/** Returns the resolved MCP configuration. */
	StartFlow = 'github.ai.chat.mcp.setup.flow',
}
...
export class McpAddConfigurationCommand {
	...
	private async getServerType(): Promise<AddConfigurationType | undefined> {
		...
		if (aiSupported) {
			...
			items.push(
				{ type: 'separator', label: localize('mcp.serverType.ai', "Model-Assisted") },
				...elligableTypes
			);
		}
