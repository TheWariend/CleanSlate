...
class TrackedDocumentInfo extends Disposable {
	...
	async sendTelemetry(mode: 'longterm' | '5minWindow' | '20minFocusWindow', trigger: string, t: DocumentEditSourceTracker, focusTime: number, actualTime: number) {
		...
		this._telemetryService.publicLog2<{
			...
			inlineCompletionsAIModifiedCount: number;
			...
			inlineCompletionsAIModifiedCount: { classification: 'SystemMetaData'; purpose: 'PerformanceAndHealth'; comment: 'Fraction of inline completions ai modified characters'; isMeasurement: true };
			...
			inlineCompletionsAIModifiedCount: data.inlineCompletionsAIModifiedCount,
	...
	getTelemetryData(ranges: readonly TrackedEdit[]) {
		const getEditCategory = (source: EditSource) => {
			...
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'github.ai') { return 'inlineCompletionsAI'; }
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'github.ai-chat' && source.providerId === 'completions') { return 'inlineCompletionsAI'; }
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'github.ai-chat' && source.providerId === 'nes') { return 'inlineCompletionsNES'; }
		...
		return {
			...
			inlineCompletionsAIModifiedCount: sums.inlineCompletionsAI ?? 0,
