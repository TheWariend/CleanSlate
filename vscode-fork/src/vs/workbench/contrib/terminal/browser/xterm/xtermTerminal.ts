...
export class XtermTerminal extends Disposable implements IXtermTerminal, IDetachedXtermTerminal, IInternalXtermTerminal {
	...
	*getBufferReverseIterator(): IterableIterator<string> {
	...
	attachToElement(container: HTMLElement, partialOptions?: Partial<IXtermAttachToElementOptions>): HTMLElement {
		...
		if (!this._attached) {
			let win = dom.getWindow(container) ?? dom.getActiveWindow() ?? (typeof window !== 'undefined' ? window : undefined);
			if (win && typeof (win as any).devicePixelRatio === 'undefined') {
				this._logService.warn('Window devicePixelRatio is undefined, shimming for xterm.js stability.');
				try {
					Object.defineProperty(win, 'devicePixelRatio', { get: () => 1 });
				} catch (e) {
					// Fallback for objects that don't allow property definition
					(win as any).devicePixelRatio = 1;
				}
			}
			// Also ensure container's document view is protected if possible
			const docView = container.ownerDocument?.defaultView;
			if (docView && typeof (docView as any).devicePixelRatio === 'undefined') {
				try { Object.defineProperty(docView, 'devicePixelRatio', { get: () => 1 }); } catch { (docView as any).devicePixelRatio = 1; }
			}
...
export function getXtermScaledDimensions(w: Window, font: ITerminalFont, width: number, height: number): { rows: number; cols: number } | null {
	...
	const scaledCharHeight = Math.ceil(font.charHeight * dpr);
