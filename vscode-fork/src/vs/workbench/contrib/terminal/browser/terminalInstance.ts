...
export class TerminalInstance extends Disposable implements ITerminalInstance {
			...
			switch (e.id) {
				...
				case TerminalCapability.CommandDetection: {
					...
					this._register(e.capability.onCommandExecuted(async (command) => {
						// Only generate ID if command doesn't already have one (i.e., it's a manual command, not AI-initiated)
	...
	private _open(): void {
		...
		const xtermHost = dom.getWindow(this._container).document.createElement('div');
