...
export const CONTEXT_BROWSER_ANNOTATION_ACTIVE = new RawContextKey<boolean>('browserAnnotationActive', false, localize('browser.annotationActive', "Whether browser annotation mode is currently active"));
...
class BrowserNavigationBar extends Disposable {
	...
	private readonly _annotationButton: HTMLButtonElement;
	private readonly _annotationLabel: HTMLElement;
	...
	constructor(
		...
		this._annotationButton = $<HTMLButtonElement>('button.browser-annotation-toggle');
		this._annotationButton.type = 'button';
		this._annotationButton.setAttribute('aria-pressed', 'false');
		this._annotationButton.title = localize('browser.annotationToggleTitle', "Annotate Page");
		this._annotationButton.appendChild(renderIcon(Codicon.add));
		this._annotationLabel = $('span.browser-annotation-label');
		this._annotationButton.appendChild(this._annotationLabel);
		...
		container.appendChild(this._annotationButton);
		...
		this._register(addDisposableListener(this._annotationButton, EventType.CLICK, () => {
			void editor.toggleAnnotationMode();
		}));
	}
	...
	setAnnotationActive(active: boolean): void {
		this._annotationButton.classList.toggle('active', active);
		this._annotationButton.setAttribute('aria-pressed', String(active));
		this._annotationLabel.textContent = active
			? localize('browser.annotationActiveLabel', "Annotating")
			: localize('browser.annotationInactiveLabel', "Annotate");
		this._annotationButton.title = active
			? localize('browser.annotationStopTitle', "Stop Annotating")
			: localize('browser.annotationStartTitle', "Annotate Page");
	}
...
export class BrowserEditor extends EditorPane {
	...
	private _annotationActiveContext!: IContextKey<boolean>;
	...
	protected override createEditor(parent: HTMLElement): void {
		...
		this._annotationActiveContext = CONTEXT_BROWSER_ANNOTATION_ACTIVE.bindTo(contextKeyService);
		...
		this._register(addDisposableListener(this._browserContainer, EventType.FOCUS, (event) => {
			...
			if (event.relatedTarget && this._model && this.shouldShowView) {
				this.runBrowserViewOperation('BrowserEditor.focus', this._model.focus());
			}
		}));
	...
	override async setInput(input: BrowserEditorInput, options: IEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<void> {
		...
		this.updateAnnotationState(this._model.annotationActive);
		...
		this._inputDisposables.add(this._model.onDidChangeNavigationState((navEvent: IBrowserViewNavigationEvent) => {
		...
		this._inputDisposables.add(this._model.onDidChangeLoadingState(() => {
			this.updateNavigationState({
				url: this._model?.url ?? '',
				canGoBack: this._model?.canGoBack ?? false,
				canGoForward: this._model?.canGoForward ?? false
			});
		...
		this._inputDisposables.add(this._model.onDidChangeAnnotationState(({ active }) => {
			this.updateAnnotationState(active);
		}));
	...
	private updateVisibility(): void {
		...
		if (this._model) {
			...
			if (show) {
				this.runBrowserViewOperation('BrowserEditor.setVisible(true)', this._model.setVisible(true));
				if (
				...
				this.window.requestAnimationFrame(() => {
					this.runBrowserViewOperation('BrowserEditor.setVisible(false)', this._model?.setVisible(false));
				});
			}
		}
	}
	...
	async toggleAnnotationMode(): Promise<void> {
		...
		try {
			if (this._model.annotationActive) {
				await this._model.stopAnnotationMode();
			} else {
				await this._model.focus();
				await this._model.startAnnotationMode();
				await this._model.focus();
			}
		} catch (error) {
			this.handleBrowserViewError('BrowserEditor.toggleAnnotationMode', error);
		}
	}
	...
	async startAnnotationMode(): Promise<void> {
		...
		try {
			...
			this.handleBrowserViewError('BrowserEditor.startAnnotationMode', error);
		}
	}

	async stopAnnotationMode(): Promise<void> {
		try {
			await this._model?.stopAnnotationMode();
		} catch (error) {
			this.handleBrowserViewError('BrowserEditor.stopAnnotationMode', error);
		}
	}

	private updateAnnotationState(active: boolean): void {
		this._annotationActiveContext.set(active);
		this._navigationBar.setAnnotationActive(active);
	}
	...
	private async doScreenshot(): Promise<void> {
		...
		try {
			...
			if (this.handleBrowserViewError('BrowserEditor.captureScreenshot', error)) {
				return;
			}
		}
	...
	private runBrowserViewOperation<T>(operation: string, promise: Promise<T> | undefined): void {
		if (!promise) {
			return;
		}
		promise.catch(error => this.handleBrowserViewError(operation, error));
	}

	private handleBrowserViewError(operation: string, error: unknown): boolean {
		if (this.isMissingBrowserViewError(error)) {
			this.cancelScheduledScreenshot();
			this.updateAnnotationState(false);
			this.logService.warn(`${operation}: browser view is no longer available.`);
			return true;
		}
	...
	private isMissingBrowserViewError(error: unknown): boolean {
		const message = error instanceof Error ? error.message : String(error);
		return message.includes('Browser view') && message.includes('not found');
	}

	forwardCurrentEvent(): boolean {
		if (this._currentKeyDownEvent && this._model) {
			this.runBrowserViewOperation('BrowserEditor.dispatchKeyEvent', this._model.dispatchKeyEvent(this._currentKeyDownEvent));
			return true;
		}
		return false;
	}
	...
	layoutBrowserContainer(): void {
		if (this._model) {
			...
			this.runBrowserViewOperation('BrowserEditor.layout', this._model.layout({
	...
	override clearInput(): void {
		...
		this.runBrowserViewOperation('BrowserEditor.stopAnnotationMode', this._model?.stopAnnotationMode());
		...
		this._annotationActiveContext.reset();
		this._navigationBar.setAnnotationActive(false);
