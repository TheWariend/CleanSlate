...
import { BrowserEditor, CONTEXT_BROWSER_CAN_GO_BACK, CONTEXT_BROWSER_CAN_GO_FORWARD, CONTEXT_BROWSER_DEVTOOLS_OPEN, CONTEXT_BROWSER_FOCUSED, CONTEXT_BROWSER_STORAGE_SCOPE, CONTEXT_BROWSER_ELEMENT_SELECTION_ACTIVE, CONTEXT_BROWSER_ANNOTATION_ACTIVE, CONTEXT_BROWSER_FIND_WIDGET_FOCUSED, CONTEXT_BROWSER_FIND_WIDGET_VISIBLE } from './browserEditor.js';
...
class ToggleAnnotationModeAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('browser.toggleAnnotationModeAction', 'Toggle Browser Annotation Mode'),
			category: BrowserCategory,
			f1: true,
			precondition: BROWSER_EDITOR_ACTIVE,
			toggled: CONTEXT_BROWSER_ANNOTATION_ACTIVE
		});
	}

	async run(accessor: ServicesAccessor, browserEditor = accessor.get(IEditorService).activeEditorPane): Promise<void> {
		if (browserEditor instanceof BrowserEditor) {
			await browserEditor.toggleAnnotationMode();
		}
	}
}
