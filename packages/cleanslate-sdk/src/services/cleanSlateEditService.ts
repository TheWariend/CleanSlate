...
import { IMarker, IMarkerHost, MarkerSeverity } from '../host/diagnostics.js';
...
export class CleanSlateEditService {
	...
	public static planEdits(model: ISlateTextModel, request: CleanSlateEditPlanRequest): CleanSlateEditPlanResult {
	...
	public static computeEdits(model: ISlateTextModel, request: CleanSlateEditPlanRequest): { range: Range, text: string }[] {
	...
	public static buildRecoveryHint(diagnostic?: CleanSlateEditDiagnostic, model?: ISlateTextModel): string {
	...
	private static planStructuredEdits(model: ISlateTextModel, edits: CleanSlateStructuredEdit[], hasVersionGuard: boolean): CleanSlateEditPlanResult {
	...
	private static getMinimumConfidenceForExact(model: ISlateTextModel, edit: CleanSlateStructuredEdit): number {
		if (!this.isCodeLikeResource(model.uri)) {
			return 0.7;
		}
	...
	private static requiresOriginalTextForRange(model: ISlateTextModel, edit: CleanSlateStructuredEdit, range: Range, hasVersionGuard: boolean): boolean {
	...
	private static scoreRangeConfidence(model: ISlateTextModel, edit: CleanSlateStructuredEdit, range: Range, hasVersionGuard: boolean): number {
		let confidence = 0.78;
		if (hasVersionGuard) {
			confidence += 0.08;
		}
	...
	private static findOverlapDiagnostic(model: ISlateTextModel, plannedEdits: CleanSlatePlannedEdit[]): CleanSlateEditDiagnostic | undefined {
	...
	private static findStructuralIntegrityDiagnostic(model: ISlateTextModel, plannedEdits: CleanSlatePlannedEdit[]): CleanSlateEditDiagnostic | undefined {
	...
	private static shouldValidateStructuralIntegrity(model: ISlateTextModel, plannedEdits: CleanSlatePlannedEdit[]): boolean {
		if (!this.isCodeLikeResource(model.uri)) {
			return false;
		}
	...
	private static computePreviewContent(model: ISlateTextModel, plannedEdits: CleanSlatePlannedEdit[]): string {
	...
	public static captureMarkerValidationBaseline(markerService: IMarkerHost, resource: URI): CleanSlateMarkerValidationBaseline {
	...
	public static async rollbackModelToContent(model: ISlateTextModel, expectedContent: string): Promise<void> {
		try {
			// Optional: a headless model has no undo stack, and the direct
			// restoration below is the fallback either way.
			await model.undo?.();
		} catch {
	...
	private static computeSmartDiff(model: ISlateTextModel, newContent: string): { range: Range, text: string }[] {
