...
## Copied with mechanical changes

Each carries its original Microsoft copyright header plus a note naming its
source path. The vendoring script rewrites imports for the SDK layout and converts
`export const enum` to `export enum` for `isolatedModules` compatibility.
...
`path.ts` is carried across rather than replaced
...
Five modules provide the subset of functionality needed by the SDK without
pulling in additional editor dependencies.
...
| `core/resources.ts` | `vs/base/common/resources.ts` | Six functions (`basename`, `dirname`, `joinPath`, `normalizePath`, `relativePath`, `isEqualOrParent`) use the vendored path module; see the file header for the compatibility assumptions. |
